...
public class frmMain extends javax.swing.JFrame
{
  ...
  private void initComponents()
  {
    ...
    jPanel4.setBorder(BorderFactory.createTitledBorder(null, "Playslist", 0, 0, new Font("Agency FB", 1, 14)));
      ...
      try
      {
        ...
        for (int i = 0; i < files.length; i++)
        {
          ...
          if ((baseFileFormat instanceof org.tritonus.share.sampled.file.TAudioFileFormat))
          {
            ...
            Playlist.setName(val);
            
            key = "author";
            val = (String)properties.get(key);
            Playlist.setAutor(val);
            
            key = "album";
            val = (String)properties.get(key);
            Playlist.setAlbum(val);
            
            key = "duration";
            Long valLong = (Long)properties.get(key);
            Playlist.setTempo(Long.valueOf(valLong.longValue() / 1000000L));
            
            Playlist.setPath(files[i].getAbsolutePath());
            
            modeloMusicas.addMusica(Playlist);
          }
  ...
  private void btRemoveMusicaActionPerformed(ActionEvent evt) {
    ...
    if (linha.intValue() > -1)
    {
      ...
      if ((lbTocandoAgora.getText().equals(modeloMusicas.getMusica(linha.intValue()).getName().toString())) && (lbTituloTocando.getText().equals("Tocando:")))
      {
  ...
  private void btMisturaActionPerformed(ActionEvent evt)
  {
    ...
    if (tbPlayListAtual.getSelectedRow() > -1)
    {
      ...
      lbTocandoAgora.setText(modeloMusicas.getMusica(tbPlayListAtual.getSelectedRow()).getName());
    }
  }
  ...
  private void tbPlayListAtualMouseClicked(MouseEvent evt)
  {
    ...
    if (linha.intValue() > -1)
    {
      ...
      {
        lbTocandoAgora.setText(modeloMusicas.getMusica(linha.intValue()).getName());
        ...
        public void run()
        {
          ...
          while ((!stop.booleanValue()) && (line.intValue() < modeloMusicas.getRowCount())) {
            try
            {
              ...
              lbTocandoAgora.setText(modeloMusicas.getMusica(line.intValue()).getName());
